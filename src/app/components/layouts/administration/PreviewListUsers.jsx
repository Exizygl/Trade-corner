          ...
          function (res) {
            if (res.status === 200) {
              ...
              alert("Il n'y a pas d'utilisateurs dans la base de donnée !")
                ...
                if (res.data.length < 3){ //Si il y a 1 ou 2 utilisateurs dans la BDD
                  for (let i=0; i<res.data.length; i++) { //pour la preview on récupére tous les utilisateurs
                    ...
                    usersTemp.push(user);
                    console.log("reponse : " + JSON.stringify(res.data[0].name));
                    console.log("longueur tableau : " + JSON.stringify(res.data.length));
                  };    
                }
                else {
    ...
    <div>
        <div className="flex  flex-row ml-12 flex-wrap">
            {state.usersPreview.map( user => 
             <Link to={`/administration/user/${user.id}`} key={user.id} ><CardUser key = {user.id} name = {user.name} role = {user.role} imageProfilUrl = {user.imageProfilUrl}/></Link>
            )}
        </div>
        <p>
